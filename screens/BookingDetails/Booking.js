  ...
  const BookingApi = useCallback(async () => {
    ...
    try {
      let response = await ApiService.PostMethode(
        'bookings/get_bookings_by_customer_phone_number',
        {phone_number: userData},
      );
      console.log('response', response);
      setLoader(false);
      let apiValue = response.data;
      let reverse=apiValue.reverse()
      setBookingData(reverse);
          ...
          <View style={styles.wrapper}>
            <FlatList
              ...
              ListEmptyComponent={() => {
                return (
                  <View
                    ...
                    <Image
                      source={require('../../Assets/images/gif/notFound.gif')}
              ...
              renderItem={({item, index}) => {
                let parse = JSON.parse(item.b_c_service_name);
                  ...
                  <>
                    <TouchableOpacity
                      ...
                      <View
                        ...
                        <Text
                          style={{...Fonts.blackColor16Bold, color: '#F9B551'}}>
                          ID: #{item._id.slice(0, 20)}
                        </Text>
                        <View
                      ...
                      <View
                        ...
                        <Text>
                          <Text style={{color: '#F9B551'}}>₹</Text>{' '}
                          {item.b_c_amount}
                        </Text>
                      </View>
                      <>
                        {parse.map(item => {
                            console.log("itemmmm",item.serviceName);
                          return (
